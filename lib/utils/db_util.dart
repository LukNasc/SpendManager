...
class DbUtil {
  ...
  static void _createDb(sql.Database db) {
    ...
    db.execute("""
     CREATE TABLE transacao (id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  static Future<List<Map<String, dynamic>>> getDataWhere(
    ...
    final db = await DbUtil.database();
    final data = await db.query(table, where: where, whereArgs: args);
    return data.toList();
  }

  static Future<void> executeSql(String sql, List<dynamic> args) async {
    final db = await DbUtil.database();
